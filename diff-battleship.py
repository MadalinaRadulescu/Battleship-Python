    ...
    while True:
        ...
        try:
            if player_versus == "1":
                ...
                difficulty_of_AI = input(
                    "Please select the difficulty of AI\n 1- I want to have fun\n 2- DESTROY ME!\n"
                )
...
def get_ships_number():
    ...
    for ship in range(int(ships_number)):
        ...
        while True:
            ship_length = input(
                f"How big you want the ship number {n}/{ships_number} (1-4)?\n"
            )
            try:
...
def get_turns_to_play(ships_lenght):
    ...
    while True:
        turns = input(
            f"How many turns would you like to play? Choose a number between {minimum_turns} and 50\n"
        )
        try:
            int_turns = int(turns)
    ...
    while True:
        ...
        try:
            if player_cordinates:
                try:
                    if (
                        player_cordinates[0] in letters[:board_size]
                        and int(player_cordinates[1:]) in numbers
                    ):
                        return int(ord(player_cordinates[0]) - ord("A")), int(player_cordinates[1:]) - 1
...
def place_ships_human(ships, board, board_size, player_versus, counter):
    ...
    for ship in ships:
        while True:
            ...
            ship_direction, valid_directions = validate_ship_position(
                row, column, board, ship
            )
            if (
                ship_direction
                    ...
                    try:
                        ...
                        if user_choice in valid_directions or len(ship) == 1:
                            if user_choice == "0":
                                ...
                                c += 1
                                ships_coordinates_on_map.append(
                                    ships_placed_coordinates
                                )
                            if user_choice == "1":
                                ships_placed_coordinates = []
    ...
    while True:
        ...
        try:
            ...
            for i in ship:
                ...
                c += 1
            if (
                valid_position == len(ship)
                and row + 1 < len(board[0])
                and row + 2 < len(board[0])
            ):
                ship_direction += "\n1-Down"
                valid_directions += "1"
        ...
        try:
            ...
            if (
                valid_position == len(ship)
                and column + 1 < len(board[0])
                and column + 2 < len(board[0])
            ):
                ship_direction += "\n2-Right"
                valid_directions += "2"
...
def update_board_after_shoot_human(
    player_board, guess_board, board_size, player_versus, counter, ships_length
):
...
def update_board_after_shoot_AI(
    player_board,
    guess_board,
    board_size,
    player_versus,
...
def print_paralel_board(
    board_size, player_one_guess_board, player_two_guess_board, name, name_two
):
    ...
    for i in player_one_guess_board:
        ...
        print(
            str(chr(letters + c))
            + "  "
            + " ".join(player_one_guess_board[n])
            + "    "
            + str(chr(letters + c))
            + "  "
            + " ".join(player_two_guess_board[n])
        )
        c += 1
        n += 1
...
def play_with_AI(
    ...
    if player_versus == "2":
        if difficulty_of_AI == "1":
            ...
            while True:
                row, column = randint(0, board_size), randint(0, board_size)
...
def perimeter_verification_AI(
    ...
    row = targeted_ship_coordinates[-1][0]
...
def main():
    ...
    # to remove, not needed cause it is called later - updated removed ---Adam--- 6.Mai.2022-code review

    counter = 0
    if player_versus == "1":
        name = input("What's your name, player one?\n")
        print(f"{name}, it is time to make your tactics")
        player_one_board, ships_one_coordinates_on_map = place_ships_human(
            player_one_ships, player_one_board, board_size, player_versus, counter
        )

        name_two = input("What's your name, player two?\n")
        print(f"{name_two}, it is time to make your tactics")
        player_two_board, ships_two_coordinates_on_map = place_ships_human(
            player_two_ships, player_two_board, board_size, player_versus, counter
        )
        ...
        player_two_board, ships_two_coordinates_on_map = place_ships_AI(
            player_two_ships,
            player_two_board,
            board_size,
            player_versus,
        ...
        name = "Anos Voldigoad"
        player_one_board, ships_one_coordinates_on_map = place_ships_AI(
            player_one_ships,
            player_one_board,
            board_size,
            player_versus,
    ...
    while True:
        ...
        if player_versus == "1":
            if counter % 2 == 0:
                ...
                update_board_after_shoot_human(
                    player_two_board,
                    player_two_guess_board,
                    board_size,
                    player_versus,
